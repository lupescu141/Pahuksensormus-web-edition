...
function laksiaisjuhlat(pelaaja) {
  ...
  console.log(
      'Uudentoivon kylä järjestää sydämelliset läksiäisjuhlat, ja koko kylä on yhtynyt iloisen tunnelman valtaan. Osallistut juhliin ja liityt kyläläisten joukkoon juhlan huumassa. Ole kuitenkin varovainen, sillä liiallinen juhlinta saattaa tuoda mukanaan ikäviä seurauksia!');
  ...
  if (valinta === '1') {
    console.log(
        'Valinta 1: Päätät osallistua juhliin hillitysti ja ottaa osaa kylän iloiseen tunnelmaan, ansaitset paikallisten suosion. Kyläläiset muistavat ystävällisyytesi ikuisesti, ja saat heiltä mukaasi arvokkaita esineitä.');
    // MUOKATAAN TÄTÄ MITÄ PELAAJA SAA
    pelaaja.esine();
  } else if (valinta === '2') {
    // Valinta 2:
    console.log(
        'Valinta 2: Annat juhlavan tunnelman viedä mukanaan ja juot liikaa, menetät otteesi todellisuudesta. Seurauksena kyläläiset menettävät kunnioituksen sinuun. Menetit 5 HP voipuessa krapulasta ja maineesi on mennyt.');
    pelaaja.hp -= 5;
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
  }
...
function kylanKummallinenKojukauppias() {
  ...
  switch (valinta) {
    case '1':
      console.log(
          'Hyvä valinta! Saat mukaasi harvinaisen esineen, sait juuri matkaasi mukaan uuden taikavoiman!');
      // LISÄTÄÄN TÄHÄN JOTAIN MITÄ PELAAJA SAA
      pelaaja.esine();
      break;
    case '2':
      console.log(
          'Voi ei! Valintasi ei ollut fiksu, menetät arvokkaita resursseja, menetit nimittäin juuri yhden taikapotionin!');
...
function ruoholaaksonNiityt(pelaaja) {
  console.log(
      'Astuttuasi Ruoholaaksoon, laakso on täynnä niittyjä ja salaperäisiä kukkia. Laakson sydämessä asuu tappavan kaunis syöjätär demoni, jonka lumoava voima vetää puoleensa uteliaita seikkailijoita. Seuraat niittyjen huumaavaa tuoksua..');
...
function kukkienHurmio(pelaaja) {
  ...
  console.log(
      'Huomaat lumoavien kukkien tuoksun, joka kutsuu sinua kohti piilotettua niittyä. Keskellä laaksoa avautuu kukkien ympäröimä paikka, jossa voit kokea niiden lumoavan voiman..');
  ...
  if (valinta1 === '1') {
    console.log(
        'Astut varovasti kukkien keskelle ja tunnet niiden lumoavan voiman ympärilläsi. Saat taitoja harhauttamiseen ja lumoaviin puheisiin. Kukat suojelevat sinua vihollisilta lyhyen aikaa. Saat matkaasi mukaan 10 eliksiiriä.');
    //ELIKSIIREJÄ VAI MUUTA? MUOKATAAN
    pelaaja.eliksiirit += 10;
  } else if (valinta1 === '2') {
    // Valinta 2:
    console.log(
        'Poimimalla kukkia huolettomasti, kukkien henki herää ja ne kutsuvat syöjättären luokseen. Joudut taisteluun syöjättären kanssa!');
...
function varjojenPiilo(pelaaja) {
  ...
  console.log(
      'Kohtaat syöjättärien salaperäisen piilon, joka näyttää olevan täynnä vaarallisia petoja ja kavalaa voimaa. Voit joko liittyä varjojen tanssiin tai pysyt varuillasi ja tarkkailet sivusta.');
  ...
  if (valinta2 === '1') {
    ...
    // Valinta 2:
    console.log(
        'Pysyt varjojen ulottumattomissa, mutta ne syöjättäret huomaavat sinut! Varjot luovat illuusioita, jotka hämmentävät sinua ja aiheuttavat tilapäistä sokeutta. Vajoat syvään uneen ja menetät yhden päivän matkastasi.');
...
function lumotunRiipuksenArvoitus(pelaaja) {
  ...
  console.log(
      'Löydät laakson keskeltä demonisen riipuksen, joka kiinnittää huomiosi voimakkaaseen energiaan. Riipus näyttää sisältävän piilotetun viestin, ja voit joko yrittää selvittää sen tai ohittaa sen.');
  ...
  if (valinta3 === '1') {
    console.log(
        'Valinta 1: Yrität ymmärtää viestin, riipus paljastaa sinulle tulevaisuuden näkymiä ja vihjeitä seuraavista vaiheista. Saat etulyöntiaseman tulevissa kohtaamisissa. Saat lisää taitoja matkaasi!');
    // LISÄTÄÄN TÄHÄN JOTAIN MITÄ PELAAJA SAA
    pelaaja.taitopiste();
  } else if (valinta3 === '2') {
    // Valinta 2:
    console.log(
        'Valinta 2: Ohitat riipuksen, mutta sen voima koskettaa sinua. Joudut hetkellisesti ajan vääristymisen uhriksi, menettäen näkemyksen ajasta ja paikasta. Demoni syöjätär ui mielesi sopukoihin ja hypnotisoi sinut lumouksiinsa, menetät 10 HP.');
    pelaaja.hp -= 10;
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
  }
...
function velhotorniEvent(pelaaja) {
  console.log(
      'Astuttuasi lähemmäs Velhotornia, huomaat sen olevan täynnä salaisuuksia ja vaaroja. Tornissa asustaa ilkeä velho. Torni on täynnä salakäytäviä ja ansoja. Eksytkö käytävien uumeniin vai kohtaatko mahdollisesti velhon. Onnea matkaan!');
...
function velhonKaksintaistelu(pelaaja) {
  ...
  console.log(
      'Saavut tornin huipulle, Velhon mahtava hahmo seisoo edessäsi, ja hänen katseensa paljastaa vuosisatojen viisauden ja taikuuden. Hän kohottaa kättään ja haastaa sinut kaksintaisteluun voimien mittelöimiseksi, onnea matkaan!');
  ...
  if (valinta3 === '1') {
    ...
    // Valinta 2:
    console.log(
        'Valinta 2: Velho suuttuu pakoyrityksestäsi ja lähettää sinua kohti voimakkaan taian. Onnistut välttämään suurimman osan hyökkäyksestä ja pääset karkuun. Tornin pitkät ja kapeat portaat saavat sinut kaatumaan ja putoat tornin syvyyksiin. Nolona jatkat matkaa… Menetät 10HP.');
    pelaaja.hp -= 10;
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
  }
...
function variksenkielenLoitsut(pelaaja) {
  ...
  if (valinta1 === '1') {
    ...
    // Valinta 2:
    console.log(
        'Valinta 2: Varikset lentelevät ympärilläsi ja nokkivat naamasi verille, vaivut syvään uneen, menetät yhden päivän matkastasi!');
    pelaaja.paivat -= 1;
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
  }
...
function varjorituaalit(pelaaja) {
  ...
  if (valinta2 === '1') {
    console.log(
        'Valinta 1: Liityt varjoryhmän rituaaliin, ryhmä yllättyy taidoistasi ja saat heiltä täyden HP:n voiman.');
...
function taikaesineenLoytaminen(pelaaja) {
  ...
  console.log(
      'Tutkiessaan rämettä, huomaat heikosti loistavan valon. Lähemmäksi siirtyessäsi löydät piilotetun kolon, jossa sijaitsee vanha magialla täytetty riipus. Pystytkö käyttämään tätä taikaesinettä hyödyksesi?');
  ...
  if (valinta3 === '1') {
    ...
    // Valinta 2:
    console.log(
        'Valinta 2: Et kunnioittanut taikaesinettä ja yritit pakottaa sen voimat ulos, herätit varisvihan, ja joudut taisteluun varisten herran kanssa! Ohjaa taisteluun!');
...
function noitataloTaistelu(pelaaja) {
  ...
  if (valinta1 === '1') {
    console.log(
        'Valinta 1: Hiippailet keittiössä ilman kiinnijäämistä, löydät salaisen komeron, jossa säilytetään arvokkaita taika-aineita. Löydät taikapotioneita!');
    // MITÄ PELAAJA SAA, MUOKATAAN
    pelaaja.potion += 10;
  } else if (valinta1 === '2') {
    // Valinta 2:
    console.log(
        'Valinta 2: Jäit kiinni ja kolmipäinen noita huomaa sinut, taistelu alkaa! Ohjaa taisteluun!');
...
function noitatalonKirjasto(pelaaja) {
  ...
  if (valinta2 === '1') {
    ...
    // Valinta 2:
    console.log(
        'Valinta 2: Jos häiritset liikaa hämähäkin seittiverhoa ja kolmipäinen noita ilmaantuu, hän haastaa sinut taisteluun! Ohjaa taisteluun!');
    // OHJAA TAISTELUUN NOIDAN KANSSA, MUOKKAA FUNKTIO!
    avaa_taistelu_ikkuna();
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
...
function sammakkojarvi(pelaaja) {
  console.log(
      'Taianomainen sammakkojärvi herättää uteliaisuuden ja seikkailunhalun. Sammakkojärven rannat täyttyvät satojen sammakoiden kurnutuksesta, kun ne kokoontuvat yhteen valtaamaan ilman.');
...
function lumoavaJarvenpeili(pelaaja) {
  ...
  console.log(
      'Järven pinnalla näet heijastuksen, joka ei näytä perinteiseltä veden heijastukselta. Katsot tarkemmin ja huomaat, että heijastuksesta ilmestyy jotain. Yhtäkkiä järvestä pompahtaa sammakko, joka ehdottaa sinulle suudelmaa.');
  ...
  if (valinta2 === '1') {
    console.log(
        'Valinta 1: Suutelet intohimoisesti sammakkoa, ja hän muuttuu kauniiksi prinssiksi! Hän kiittää sinua pelastuksesta ja paljastaa olevansa lumottu prinssi.');
    console.log(
        'Prinssi kertoo, että hänet muutettiin sammakoksi pahan velhon toimesta, ja hän etsii keinoa kääntää lumous kokonaan.');
  } else if (valinta2 === '2') {
    // Valinta 2:
    console.log(
...
function keijukaistenKuningatar(pelaaja) {
  ...
  console.log(
      'Astelet lähemmäksi suohiisin keijukaiskuningatarta, jonka valo saa koko laakson kimaltelemaan.');
  ...
  if (valinta1 === '1') {
    ...
    console.log(
        'Jäät katsomaan kaunista valoa, sydämesi täyttää suru ja menetit osan voimistasi, itku täyttää silmäkulmasi, matka jatkuu....');
    pelaaja.hp -= 10;
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
  }
...
function seuraaTulikarpastenValoa(pelaaja) {
  ...
  if (valinta2 === '1') {
    console.log(
        'Valinta 1: Onnistuit seuraamaan tulikärpäsiä, ne opastavat sinut piilotetulle aarteelle suohiisien keskellä.');
    console.log(
        'Löydät lumoavia esineitä ja voimakasta taikaa. Sait 10 eliksiiriä!');
    // ELIKSIIRI VAI JOTAIN MUUTA, MUOKKAA
    pelaaja.eliksiiri();
  } else if (valinta2 === '2') {
    // Valinta 2:
    console.log(
        'Valinta 2: Tulikärpäset johdattavat sinut suohiisin salaiseen paikkaan, tämä oli ansa!');
...
function suonLumous(pelaaja) {
  ...
  console.log(
      'Päätät olla liittymättä suon syvempiin syvyyksiin ja astut varoen sen reunoille.');

  // Valinnat:
  console.log('Valinnat:');
  const valinta3 = prompt(
      'Valinta 1: Kävelet reunoja pitkin.\nValinta 2: Olet uhkarohkea ja hyppelet kiviä pitkin');

  // Valinta 1:
  if (valinta3 === '1') {
    console.log(
        'Valinta 1: Pysyt suon reunoilla ja olet varovainen. Löydät suon uumenista kasan eliksiirejä ja piilotetun reitin ulos.');
    console.log(
        'Mahtavaa, et eksynyt tällä kertaa! Löysit vielä kasan eliksiirejä!');
    // ELIKSIIRIT VAI JOTAIN MUUTA??
    pelaaja.eliksiiri();
  } else if (valinta3 === '2') {
    // Valinta 2:
    console.log(
        'Valinta 2: Hyppelehtiessäsi kivillä huomaat salaperäisen portin suossa.');
    console.log(
        'Kompastut ja tipahdat portista toiseen ulottuvuuteen. Kohtaat painajaisena elämäsi traagisimmat tapahtumat. Menetit 5HP');
    pelaaja.hp -= 5;
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
  }
...
function peikkoluola(pelaaja) {
  console.log(
      'Pimeän metsän kätköissä sijaitseva peikkoluola kuhisee salaisuuksia ja vaaroja.');
...
function peikkokuningas(pelaaja) {
  ...
  console.log(
      'Seikkailija tutkii peikkoluolaa ja toivoo löytävänsä peikkokuninkaan, joka vartioi syvimmän kammion aarteita.');
  ...
  if (valinta1 === '1') {
    ...
    // Valinta 2:
    console.log(
        'Valinta 2: Rohkeana seikkailijana päätät kohdata peikkokuninkaan suoraan.');
    console.log(
        'Astut esiin varjoista ja seisot hänen edessään. Peikkokuningas nauraa mahtipontisesti ja haastaa sinut taisteluun!');
...
function hamahakkikuningatar(pelaaja) {
  ...
  if (valinta2 === '1') {
    ...
    console.log(
        'Kammiossa roihahtaa liekkejä, ja hämähäkkikuningatar kiroaa sinut. Kuulet vaikeroivat tuskan huudot takanasi kun pakenet luolasta…');
...
function lohikaarmeenPesa(pelaaja) {
  ...
  console.log(
      'Peikkoluolan uumeniin kätkeytyy lohikäärmeen pesä, jonka peikot ovat vanginneet.');
  ...
  if (valinta3 === '1') {
    ...
    console.log(
        'Jatkat haikeana matkaasi ja toivot, että törmäät lohikäärmeeseen vielä joku päivä… Sait esineitä.');
    ...
    console.log(
        'Pian kuulet, että joku toinen rohkea seikkailija on lähtenyt lohikäärmeen pesälle ja saanut haltuunsa uskomattomia aarteita. Kieltämättä vituttaa eikö. Ja matka jatkuu.');
  } else {
    console.log('Virheellinen syöte, valitse uudelleen!');
  }
}